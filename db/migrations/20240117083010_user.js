module.exports.up = async (knex) => {
  ...
  await knex.schema.createTable("voyages", (table) => {
    table.increments("id").primary();
    table.integer("idDepartureStation").notNullable().references("id").inTable("stations");
    table.integer("idArrivalStation").notNullable().references("id").inTable("stations");
  ...
  await knex.schema.createTable("reservations", (table) => {
    table.increments("id").primary();
    table.text("matriculeUser").notNullable().references("matricule").inTable("users");
    table.text("matriculeManager").notNullable().references("matricule").inTable("users");
